...
// ==================== PATHWAY CLASSIFICATION (NEW) ====================

export type PathwayClassificationKey = "type-a" | "type-b" | "type-c";

export interface PathwayClassificationResult {
  key: PathwayClassificationKey;
  label: "Type A" | "Type B" | "Type C";
  title: string;
  shortTitle: string;
  summary: string;
  rationale: string;
...
function getPathwayComplexitySignals(plan: PlanData): {
  shortlistCount: number;
  hasPathway: boolean;
  hasTreatmentGoal: boolean;
  hasFamilyStructure: boolean;
  hasDonorNeed: boolean;
  hasSurrogateNeed: boolean;
  hasConstraints: boolean;
  hasPriorities: boolean;
  hasTimeline: boolean;
  hasActiveTimeline: boolean;
  hasAdvisoryContext: boolean;
  hasAdvisoryNotes: boolean;
  hasMultipleCountries: boolean;
  hasBroadShortlist: boolean;
  pathwayValue: string;
  familyValue: string;
  treatmentGoalValue: string;
} {
  const timelineCounts = getTimelineCounts(plan?.timeline_items);
  const shortlistCount = plan?.shortlisted_countries?.length ?? 0;
  const pathwayValue = normalizeLower(plan?.pathway_type);
  const familyValue = normalizeLower(plan?.family_structure);
  const treatmentGoalValue = normalizeLower(plan?.treatment_goal);
  ...
  return {
    ...
    hasSurrogateNeed: Boolean(plan?.surrogate_needed),
    hasConstraints: countMeaningfulItems(plan?.constraints) > 0,
    hasPriorities: countMeaningfulItems(plan?.priorities) > 0,
    hasTimeline: timelineCounts.total > 0,
    hasActiveTimeline: timelineCounts.inProgress > 0 || timelineCounts.completed > 0,
    hasAdvisoryContext:
      hasMeaningfulText(plan?.advisory_status) ||
      hasMeaningfulText(plan?.advisory_next_step) ||
      hasMeaningfulText(plan?.advisory_pathway),
    hasAdvisoryNotes: hasMeaningfulText(plan?.advisory_notes),
    hasMultipleCountries: shortlistCount >= 2,
    hasBroadShortlist: shortlistCount >= 4,
    pathwayValue,
    familyValue,
    treatmentGoalValue,
  };
...
function buildTypeAClassification(plan: PlanData): PathwayClassificationResult {
  const signals = getPathwayComplexitySignals(plan);
  const indicators: string[] = [];

  if (signals.hasPathway) indicators.push("Pathway direction is defined");
  if (signals.shortlistCount === 1) indicators.push("Planning is concentrated around a single country");
  if (signals.shortlistCount === 2) indicators.push("Shortlist remains narrow and directed");
  if (signals.hasTimeline) indicators.push("Timeline structure is already established");
  if (!signals.hasDonorNeed && !signals.hasSurrogateNeed) indicators.push("No donor or surrogacy complexity is currently indicated");
  if (!signals.hasConstraints) indicators.push("Constraint profile appears relatively light at this stage");
  ...
  return {
    ...
    shortTitle: "Directed",
    summary:
      "Your case appears relatively directed. The primary need is validation, sequencing, and disciplined execution rather than broad comparative restructuring.",
    rationale:
      "This classification is used when the plan already has a clear direction, limited branching complexity, and fewer cross-border variables competing at once.",
    nextFocus:
      "Pressure-test the current direction, confirm jurisdiction fit, and tighten the execution sequence so progress is not lost through avoidable gaps.",
    advisoryFit:
      "Best suited to a focused strategy session or concise advisory review centered on validation and next-step precision.",
    complexity: "low",
    indicators,
  };
}

function buildTypeBClassification(plan: PlanData): PathwayClassificationResult {
  const signals = getPathwayComplexitySignals(plan);
  const indicators: string[] = [];

  if (signals.hasMultipleCountries) indicators.push("More than one country remains under active consideration");
  if (signals.shortlistCount >= 2 && signals.shortlistCount <= 4) indicators.push("A comparative shortlist is in place");
  if (signals.hasConstraints) indicators.push("Constraints need to be weighed against jurisdiction tradeoffs");
  if (signals.hasPriorities) indicators.push("Decision quality depends on balancing stated priorities");
  if (!signals.hasActiveTimeline) indicators.push("Execution sequencing still needs stronger activation");
  if (signals.hasAdvisoryContext) indicators.push("Advisory framing is beginning to emerge");

  return {
    key: "type-b",
    label: "Type B",
    title: "Comparative Case",
    shortTitle: "Comparative",
    summary:
      "Your case appears comparative. The main challenge is not a lack of options, but deciding between plausible options with enough structure to avoid weak tradeoff decisions.",
    rationale:
      "This classification is used when multiple jurisdictions or planning routes remain viable and the central need is structured comparison, prioritization, and sequencing clarity.",
    nextFocus:
      "Narrow ambiguity by comparing shortlist options against pathway fit, timeline implications, legal friction, and practical execution demands.",
    advisoryFit:
      "Best suited to structured comparative advisory work where decision support is more valuable than generic information gathering.",
    complexity: "moderate",
    indicators,
  };
}

function buildTypeCClassification(plan: PlanData): PathwayClassificationResult {
  const signals = getPathwayComplexitySignals(plan);
  const indicators: string[] = [];

  if (signals.hasDonorNeed) indicators.push("Donor-related variables increase pathway complexity");
  if (signals.hasSurrogateNeed) indicators.push("Surrogacy introduces added legal and sequencing complexity");
  if (signals.hasConstraints) indicators.push("Constraint profile suggests higher-friction decision making");
  if (signals.hasBroadShortlist) indicators.push("A broad shortlist increases comparative burden");
  if (signals.hasFamilyStructure) indicators.push("Family structure likely affects jurisdiction fit and execution design");
  if (signals.hasAdvisoryNotes) indicators.push("Existing advisory notes indicate active complexity or unresolved variables");
  ...
  return {
    ...
    shortTitle: "Complex",
    summary:
      "Your case appears structurally complex. The planning challenge includes higher-friction variables, greater coordination risk, or legal and logistical layers that benefit from deeper strategic support.",
    rationale:
      "This classification is used when donor, surrogate, family-structure, legal, timing, or constraint-driven factors substantially increase decision complexity and sequencing sensitivity.",
    nextFocus:
      "Reduce hidden risk by clarifying dependencies, narrowing uncertainty, and validating each major decision against legal, operational, and cross-border realities.",
    advisoryFit:
      "Best suited to deeper advisory engagement where interpretation, sequencing, and risk containment matter as much as selection itself.",
    complexity: "high",
    indicators,
  };
}

export function determinePathwayClassification(
  plan: PlanData
): PathwayClassificationResult {
  const signals = getPathwayComplexitySignals(plan);

  const complexityScore =
    (signals.hasDonorNeed ? 2 : 0) +
    (signals.hasSurrogateNeed ? 3 : 0) +
    (signals.hasConstraints ? 1 : 0) +
    (signals.hasBroadShortlist ? 1 : 0) +
    (signals.hasAdvisoryNotes ? 1 : 0) +
    (
      signals.familyValue.includes("single") ||
      signals.familyValue.includes("same-sex") ||
      signals.familyValue.includes("solo") ||
      signals.familyValue.includes("lgbt")
        ? 1
        : 0
    );

  const comparisonScore =
    (signals.hasMultipleCountries ? 2 : 0) +
    (signals.shortlistCount >= 3 ? 1 : 0) +
    (signals.hasPriorities ? 1 : 0) +
    (signals.hasConstraints ? 1 : 0) +
    (!signals.hasActiveTimeline && signals.hasTimeline ? 1 : 0);

  const directedScore =
    (signals.hasPathway ? 1 : 0) +
    (signals.shortlistCount > 0 && signals.shortlistCount <= 2 ? 2 : 0) +
    (signals.hasTimeline ? 1 : 0) +
    (!signals.hasDonorNeed ? 1 : 0) +
    (!signals.hasSurrogateNeed ? 1 : 0);

  if (complexityScore >= 3) {
    return buildTypeCClassification(plan);
  }

  if (comparisonScore >= 3 && directedScore < 5) {
    return buildTypeBClassification(plan);
  }

  if (signals.shortlistCount >= 2 && !signals.hasActiveTimeline) {
    return buildTypeBClassification(plan);
  }

  return buildTypeAClassification(plan);
}

export function getPathwayClassificationBadgeTone(
  classification: PathwayClassificationResult
): "emerald" | "amber" | "rose" {
  if (classification.key === "type-a") return "emerald";
  if (classification.key === "type-b") return "amber";
  return "rose";
}
